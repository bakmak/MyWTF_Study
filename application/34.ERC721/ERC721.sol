...
contract ERC721 is IERC721, IERC721Metadata {
    ...
    using Srtings for uint256;  //  使用String库
    ...
    //  实现IERC721的balanceOf，利用_balance变量查询owner地址的balance
    ...
    //  查询 spender地址是否可以使用tokenId（需要owner或被授权地址）spender, uint tokenId
    ...
    //  条件：1. tokenId被from拥有           2. to 不是0地址
    ...
    //  安全转账，安全地将 tokenId 代币从 from 转移到 to，会检查合约接收者是否了解 ERC721 协议，以防止代币被永久锁定。调用了_transfer函数和_checkOnERC721Received函数。
    //  条件：from 不能是0地址      to 不能是0地址      tokenId 代币必须存在，并且被 from拥有
    ...
    //  _checkOnERC721Received函数：用于在 to 为合约的时候调用IERC721Received-onERC721Received，以防 tokenId 被不小心转入黑洞
